...
const DISPLAY: { id: string; label: string }[] = [
  { id: 'year',          label: 'Year' },
  { id: 'make',          label: 'Make' },
  ...
  { id: 'sold_date',     label: 'Date' },      // date-only rendering
  { id: 'auction_house', label: 'House' },
  { id: 'buyer_number',  label: 'Buyer' },
  { id: 'state',         label: 'State' },
];

// Columns to actually fetch from DB (includes id for row keys)
const QUERY_COLUMNS: string[] = ['id', ...DISPLAY.map(d => d.id)];

// For sorting, allow any displayed column (plus id as fallback)
...
export default function SearchPage() {
  ...
  // Fetch whenever filters/sort/page change (debounced)
  // eslint-disable-next-line react-hooks/exhaustive-deps
