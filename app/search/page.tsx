...
const COLUMNS = [
  'id','title','make','model','year','vin','odometer',
  'wovr_status','sold_status','sold_price','auction_house',
  'stock_number','auction_number','created_at'
];
...
export default function SearchPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      if (f.sold_status) q = q.eq('sold_status', f.sold_status);
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-black/5">
              <tr>
                {[
                  ['created_at','Added'],['title','Title'],['make','Make'],['model','Model'],
                  ['year','Year'],['vin','VIN'],['odometer','Odometer'],['wovr_status','WOVR'],
                  ['sold_status','Sold Status'],['sold_price','Sold $'],['auction_house','House'],
                  ['stock_number','Stock #'],['auction_number','Auction #'],
                ].map(([id,label]) => (
                  <th key={id} onClick={()=>toggleSort(String(id))} className="px-3 py-2 text-left cursor-pointer">
            ...
            <tbody>
              {rows.length === 0 && !loading && <tr><td colSpan={13} className="p-8 text-center text-gray-400">No results.</td></tr>}
              {rows.map(r => (
                <tr key={r.id} className="border-t hover:bg-black/5">
                  <td className="px-3 py-2">{new Date(r.created_at).toLocaleString()}</td>
                  <td className="px-3 py-2 font-medium">{r.title || `${r.year||''} ${r.make||''} ${r.model||''}`}</td>
                  <td className="px-3 py-2">{r.make}</td>
                  <td className="px-3 py-2">{r.model}</td>
                  <td className="px-3 py-2">{r.year}</td>
                  <td className="px-3 py-2 font-mono text-xs break-all">{r.vin}</td>
                  <td className="px-3 py-2">{r.odometer?.toLocaleString?.() ?? r.odometer}</td>
                  <td className="px-3 py-2">{r.wovr_status || '—'}</td>
                  <td className="px-3 py-2">{r.sold_status || '—'}</td>
                  <td className="px-3 py-2">{r.sold_price!=null ? `$${Number(r.sold_price).toLocaleString()}` : '—'}</td>
                  <td className="px-3 py-2">{r.auction_house}</td>
                  <td className="px-3 py-2 font-mono text-xs">{r.stock_number}</td>
                  <td className="px-3 py-2 font-mono text-xs">{r.auction_number}</td>
                </tr>
              ))}
            </tbody>
          </table>
